...
def output_files(seqs, community, blocks, block_maps, output_map, relabelled_unimog, directory):
    new_seqs={}
    for genome in community:
        new_seqs[genome]=[]
        j=0
...
def main(unimog_filepath, relabelled_unimog, relabelled_dir, output_map):
    seqs = read_in_unimog(unimog_filepath)
    genomes = list(seqs.keys())
    blocks, block_maps = get_blocks(seqs, genomes)
    output_files(seqs, genomes, blocks, block_maps, output_map, relabelled_unimog, relabelled_dir)
...
if __name__ == "__main__":
    ...
    main(args.unimog_filepath, args.relabelled_unimog, args.relabelled_dir, args.output_map)
