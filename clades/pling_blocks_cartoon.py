...
def read_in_all_blocks(file, genomes):
    blocks = {}
    keys = ["Plasmid", "Block_ID", "Start", "End"]
    entries = [f"{genomes[i]}~{genomes[i+1]}" for i in range(len(genomes)-1)] + [f"{genomes[i+1]}~{genomes[i]}" for i in range(len(genomes)-1)]
    with open(file, "r") as f:
        prev_line = f.readline()
        for line in f:
            split = prev_line.split('\t')
            if split[0] in entries:
                entry = split[0]
                blocks_dict={keys[i]:[split[i+1]] for i in range(4)}
                entry_not_finished = True
                while entry_not_finished:
                    if "~" in line or "Plasmid" in line:
                        entry_not_finished = False
                        blocks[entry] = pd.DataFrame(blocks_dict)
                    else:
                        split = line.split('\t')
                        for i in range(4):
                            blocks_dict[keys[i]].append(split[i+1])
                        line = f.readline()
            prev_line = line
    return blocks

def convert_blocks_to_matches(blocks):
    matches = []
    rev = lambda x: x[1:] if x[0]== '-' else f"-{x}"
    for entry in blocks.keys():
        frame = blocks[entry]
        ref, query = entry.split("~")
        ref_len = frame[frame["Plasmid"]==ref]["End"].max()
        qry_len = frame[frame["Plasmid"]==query]["End"].max()
        all_blocks = list(set(frame[frame["Plasmid"]==ref]["Block_ID"].to_list()))
        for block in all_blocks:
            ref_entries = frame[(frame["Plasmid"]==ref) & (frame["Block_ID"]==block)]
            qry_entries = frame[(frame["Plasmid"]==query) & ((frame["Block_ID"]==block) | (frame["Block_ID"]==rev(block)))]
            if len(qry_entries)!=0:
                for i,j in itertools.combinations(list(ref_entries.index), list(qry_entries.index)):
                    ref_start,ref_end = [ref_entries.loc[i,"Start"], ref_entries.loc[i,"End"]]
                    if ref_entries.loc[i,"Block_ID"] == qry_entries.loc[j,"Block_ID"]:
                        qry_start,qry_end = [int(qry_entries.loc[i,"Start"]), int(qry_entries.loc[i,"End"])]
                        strand = 1
                    else:
                        qry_end,qry_start = [int(qry_entries.loc[i,"Start"]), int(qry_entries.loc[i,"End"])]
                        strand = -1
                    len_1 = ref_end-ref_start
                    len_2 = abs(qry_end-qry_start)
                    identity = 100
                    nuc_line = "\t".join([ref_start,ref_end,qry_start,qry_end,len_1,len_2,identity,ref_len,qry_len, 1, strand, ref, query])
                    matches.append(pymummer.Alignment(nuc_line))
    return matches
...
class Assemblies:
    ...
    def _make_nucmer_files(self, pling_unimogs):
        self.nucmer_matches = []
        self.nucmer_files = []
        ...
        for i in range(len(self.assemblies) - 1):
            ...
            if os.path.exists(nucmer_file):
                ...
                subprocess.check_call(f"nucmer --diagdiff 20 --breaklen 500  --maxmatch {self.fasta_files[i+1]} {self.fasta_files[i]} && delta-filter -rq out.delta > out.1delta", shell=True)
...
parser.add_argument('--nucmer_min_id', type=int, help='Minimum identity when running nucmer [%(default)s]', default=80, metavar='FLOAT')
parser.add_argument('-v', '--verbose', action='store_true', help='Be verbose')
parser.add_argument('outprefix', help='Prefix of output files')
parser.add_argument('fa_list', help='List of at least 2 fasta files')
